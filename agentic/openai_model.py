...
from agentic.utils.data_processing import robust_load_json

# Find project root and load .env file from there
# Try multiple locations to ensure .env is found
env_loaded = False
if __file__:
    # Try from module location (agentic/openai_model.py -> project root)
...
# Last fallback: try without path (will search from cwd up)
if not env_loaded:
...
class OpenAIBaseModel():
    ...
    def __init__(
        ...
        # Default to OpenAI if api_type is not explicitly "azure"
        # This allows OPENAI_API_KEY to be read even when API_TYPE is not set
        openai_vars = self.read_openai_env_vars(model_name) if api_type != "azure" else {
            "api_version":None, 
            "api_base": None,
            "api_key": None,
            "model": None
...
class OpenAIModel(OpenAIBaseModel):
    ...
    def simple_prompt(self, input_text: str):
        """
        Simple wrapper around the `prompt()` method to make it easier to use
...
if __name__ == '__main__':
    model = OpenAIModel(model_name="llama3.1:70b")
    print(model.simple_prompt('Hello'))
