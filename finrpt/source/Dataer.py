...
class Dataer:
    ...
    def get_company_report_em(self, date, stock_code):
        ...
        # Check database first for reports matching the date range
        try:
            conn = sqlite3.connect(self.database_name)
            c = conn.cursor()
            # Try to find report for previous year (Q4 annual report)
            ...
            if query_result:
                c.close()
                conn.close()
                return dict(zip(COMPANY_REPORT_TABLE_COLUMNS, query_result))
            # Try semi-annual report
            report_id_q2 = f'{stock_code}_{year}_Q2'
            c.execute('SELECT * FROM company_report WHERE report_id = ?', (report_id_q2,))
            query_result = c.fetchone()
            if query_result:
                c.close()
                conn.close()
            ...
            print(f"Error checking database for report: {e}")
            pass  # Continue to API call if database check fails
    ...
    def get_company_news_sina(self, stock_code, end_date, start_date = None, company_name=""):
        ...
        # Check database first for all news in date range
        try:
            conn = sqlite3.connect(self.database_name)
            c = conn.cursor()
            c.execute('''SELECT * FROM news WHERE stock_code = ? AND news_time BETWEEN ? AND ?''', (stock_code, start_date, end_date))
            results = c.fetchall()
            c.close()
            conn.close()
            ...
            if results:
                ...
                # Filter by news_decision = "是" if needed, but return all for now
                return result_list
        except Exception as e:
            print(f"Error checking database for news: {e}")
            pass  # Continue to API call if database check fails
