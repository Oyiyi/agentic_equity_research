...
class FinRpt:
    def __init__(self, model_name="gpt-4o", max_rounds=3, language='zh', database_name=None, save_path='./reports'):
        ...
        if database_name is None:
            # Use relative path: finrpt/source/cache.db
            if __file__:
                project_root = Path(__file__).parent.parent.parent
    ...
    def run(self, date, stock_code=None, company_name=None):
        ...
        # Handle case where trend is None (no trend data in database)
        if trend is not None and len(trend) > 3:
            data["trend"] = 1 if trend[3] > 0 else 0
        else:
            data["trend"] = 0  # Default to 0 if no trend data found
